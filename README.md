...
We developed the model code in a non-traditional manner, which may be seen as hackish. Instead of using the standard subclassing method in TensorFlow for creating custom layers, which would place the weight matrices inside the custom layer class and potentially hinder flexible debugging and understanding for new readers, we took a different approach. We designed the code to allow for more adaptable operations by defining the weight matrices within a custom layer and returning them as Keras Symbolic Tensors without any internal operation. This will make it easier for new readers to grasp the model's structure and implementation, ultimately facilitating benchmark testing and promoting efficient derivative research.
